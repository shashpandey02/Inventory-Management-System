...
public static int SN;
    ...
    public SearchRes(String res){
        ...
        getContentPane().setBackground(col);
        SN=0;
        tableFails.getColumnModel().getColumn(5).setCellRenderer(new TableActionCellRender());
        tableFails.getColumnModel().getColumn(5).setCellEditor(new TableActionCellEditor());
        ...
        try{
                ...
                while(rs.next()){
                    ...
                    if(slno.equalsIgnoreCase(res)||asset.equalsIgnoreCase(res)||eq.equalsIgnoreCase(res)){
                        ...
                        txtAsset.setText(rs.getString("assetID")); txtAsset.setEditable(false);
                ...
                model=(DefaultTableModel) tableFails.getModel();
                rs=st.executeQuery("select * from historyCard");
                
                int i=1;
                while(rs.next()){
                    if(rs.getString("ID").equals(txtID.getText())){
                        model.addRow(new Object[]{i++,rs.getString("reportDate"),rs.getString("failedDate"),rs.getString("rectifiedDate"),rs.getString("correctiveActionRefNo"),rs.getString("report")});
                        SN=rs.getInt("SN");
                    }
                }
